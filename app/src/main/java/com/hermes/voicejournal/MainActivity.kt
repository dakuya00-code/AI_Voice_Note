...
class MainActivity : AppCompatActivity() {
        ...
        dialog.setOnShowListener {
            dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
                ...
                val cfg = RecordingConfig(
                    ...
                    uploadPath = uploadPathInput.text?.toString().orEmpty().trim().ifBlank { "/webhook/voice-note/upload" },
    ...
    private fun showInstallGuideDialog() {
        val message = buildString {
            ...
            appendLine("3. 업로드 경로는 /webhook/voice-note/upload 유지")
    ...
    private fun refreshConfigSummary() {
        ...
        val summary = buildString {
            ...
            appendLine("- 전사 방식: 서버 n8n/Whisper 처리")
            appendLine("- 모바일 전사 상태: 서버 분석 사용")
            appendLine()
